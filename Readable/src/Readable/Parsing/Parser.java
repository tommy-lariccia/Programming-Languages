...
public class Parser {
    ...
    private boolean allowReturns = false;  // unnatural but unlikely to proliferate errors
    ...
    public Lexeme program() {
        Lexeme root = statementList();
        if (peek() != null)
            root.addChild(error("A statement should be either start a block (conditional, function, loop), call a function, or " +
    ...
    private Lexeme progStatement() {
        ...
        } else if (check(RETURN) && allowReturns == true) {
            root = returnStatement();
        } else if (check(IDENTIFIER)) {
            if (checkNext(OPAREN)) root = functionCall();
            else root = variableInitializationAssignment();
        } else if (variableInitializationAssignmentPending()) {
            root = variableInitializationAssignment();
        } else if (conditionalStatementPending()) {
            root = condBlock();
        } else if (loopStatementPending()) {
            root = loopStatement();
        } else if (lambdaInitializationPending()) {
            root = lambdaInitialization();
        } else {
    ...
    private Lexeme lineEnd() {
        ...
        if (peek() == NEW_LINE)
            ...
            return error("A statement should end either with a new line, or with an end to the file.");
        }
        return new Lexeme(NULL);
    }
    ...
    private Lexeme functionDefinition() {
        ...
        consume(CPAREN); consume(COLON); consume(NEW_LINE); consume(UP);
        allowReturns = true;
    ...
    private Lexeme whileLoop() {
        ...
        root.addChild(expression());
        consume(COLON); consume(NEW_LINE); consume(UP);
        root.addChild(statementList());
        consume(DOWN);
        return root;
    }
    ...
    private Lexeme condBlock() {
        Lexeme root = new Lexeme(CONDITIONAL_BLOCK);
        root.addChild(ifBlock());
        while (checkNext(ELSE) && checkNextNext(IF))
            root.addChild(elifBlock());
        if (checkNext(ELSE))
            root.addChild(elseBlock());
        return root;
    }
